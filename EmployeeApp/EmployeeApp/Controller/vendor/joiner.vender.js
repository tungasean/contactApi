var fulltext = [
    '<script src="../Controller/Scripts/jquery-1.9.1.js"></script>',
    '<script src="../Controller/Scripts/bootstrap.js"></script>',
    '<script src="../Controller/Scripts/angular.js"></script>',
    '<script src="../Controller/Scripts/angular-route.js"></script>',
    '<script src="../Controller/Scripts/angular-scenario.js"></script>',
    '<script src="../Controller/Service.js"></script>',
    '<script src="../Controller/form/index.js"></script>',
    '<script src="../Controller/form/menu.js"></script>',
    '<script src="../Controller/validate/rules.js"></script>',
    '<script src="../Controller/validate/validation.js"></script>',
    '<script src="../Controller/vendor/string.js"></script>',
    '<script src="../Controller/language/lang_vi.js"></script>',
    //initController
    '<script src="../Controller/form/edit.employee.js"></script>',
    '<script src="../Controller/form/add.employee.js"></script>',
    '<script src="../Controller/form/delete.employee.js"></script>',
    '<script src="../Controller/form/list.employee.js"></script>'

];
document.write(fulltext.join(''));
